...
| Name | Type | Default | Description |
|------|------|---------|-------------|
| `Venvpath` | Str | `` | Путь к venv (напр. `C:/work/stream_alex/.../venv`) |
| `Module` | Str | `cuda_ipc_transport` | Python module (`-m`) или script path |
| `Moduleargs` | Str | `--source test --width 512 --height 512` | Аргументы (channel-prefix подставляется автоматически из bridge) |
...
**Launch contract:**
```
{Venvpath}/Scripts/python -m {Module} --channel-prefix {bridge.Channelprefix} {Moduleargs}
```
Если `Venvpath` пуст, используется системный `python`. `--channel-prefix` подставляется автоматически из `cuda_ipc_bridge.Channelprefix` — пользователю не нужно дублировать.
...
### Новые аргументы
- `--channel-prefix ml` — заменяет `--channel`. Harness создаёт 3 канала: `{prefix}_img` (receive), `{prefix}_depth` (receive), `{prefix}_result` (send). Совместимость: `--channel` остаётся как alias для `--channel-prefix` (single channel = result only, для обратной совместимости).
...
| 4 | Stop button terminates subprocess cleanly | process exited within 5s, no zombie, SharedMemory released |
